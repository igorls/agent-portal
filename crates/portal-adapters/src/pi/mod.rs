//! Pi coding agent adapter (`pi` CLI, `@mariozechner/pi-coding-agent`).
//!
//! Store layout (verified against Pi 0.80.10):
//! `~/.pi/agent/sessions/<encoded-cwd>/<ISO-ts>_<uuid>.jsonl`.
//! Header line is `{type:"session", version:3, id, cwd}`; dialogue is a
//! parentId-linked chain of `message` records with content blocks
//! (`text` / `thinking` / `toolCall`) and separate `toolResult` roles.
//!
//! v1 is read + resume + brief-target. Native write is deferred — Pi owns
//! id generation and the parent chain; synthesizing files risks resume
//! breakage.
...
impl AgentAdapter for PiAdapter {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            launch_resume: true,
            launch_new: true,
            context_tokens: None, // model-dependent (ollama/anthropic/…)
            write_confidence: None,
            version_range_tested: "0.80.x".to_string(),
            notes: vec![
                "Sessions live under ~/.pi/agent/sessions, grouped by encoded cwd".into(),
