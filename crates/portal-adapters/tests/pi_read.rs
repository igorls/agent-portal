...
use portal_adapters::pi::PiAdapter;
...
fn fixture_store() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/pi/v0.80/store")
}

fn installation() -> Installation {
    Installation {
        cli_path: None,
        version: Some("0.80.10".into()),
        store_root: fixture_store().display().to_string(),
    }
}

#[test]
fn enumerates_project_and_session_from_header() {
...
fn reads_transcript_into_canonical_ir() {
    ...
    let session = adapter
        .read_session(
            &installation(),
            &SessionLocator {
                native_id: "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb".into(),
                store_path: None,
            },
        )
        .expect("read session");

    assert_eq!(session.identity.agent_id, "pi");
    ...
    assert!(matches!(
        content[0].blocks[0],
        Block::Text { ref text } if text.contains("Pi adapter")
    ));

    let assistant = content[1];
    assert_eq!(assistant.role, Role::Assistant);
    assert_eq!(assistant.model.as_deref(), Some("glm-5.2:cloud"));
    ...
    assert!(matches!(
        ...
        Block::ToolCall {
            ...
            ref name,
            ..
        } if call_id == "call_bash1" && name == "bash"
    ));

    assert_eq!(content[2].role, Role::Tool);
    assert!(matches!(
        content[2].blocks[0],
        Block::ToolResult { ref call_id, .. } if call_id == "call_bash1"
    ));

    assert!(session.usage.known);
    assert_eq!(session.usage.input_tokens, 900);
    assert_eq!(session.usage.output_tokens, 120);
...
fn resume_and_new_commands() {
    ...
    let fresh = adapter
        ...
        .expect("new");
    assert_eq!(fresh.program, "pi");
    assert_eq!(fresh.args, vec!["continue the work".to_string()]);
    assert_eq!(fresh.cwd, r"P:\demo\app");
}
